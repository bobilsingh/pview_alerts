...
<?php if ($view === 'list') { ?>
  <div class="page-head">
    <div>
      <h2>Roles</h2>
      <div class="subtitle">Define operator and administrator roles.<br> Each role gets its own tab in the Module Permissions panel and an admin-tier flag controlling system-wide ticket scope.</div>
    </div>
        ...
        <tbody>
          <?php foreach ($roles as $r) { ?>
            <tr>
              ...
              <td class="text-end">
                <a class="btn btn-sm btn-light" href="<?= site_url('roles/edit/' . esc($r['role_key'])); ?>" title="Rename label">
                  <i class="bi bi-pencil"></i>
                </a>
                <?php if ((int) $r['is_builtin'] !== 1) { ?>
                  <a class="btn btn-sm btn-outline-danger"
                    href="<?= site_url('roles/delete/' . esc($r['role_key'])); ?>"
                    data-method="post"
      ...
      <form method="post" action="<?= esc($action); ?>" data-loading-form="1">
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" name="role_key" class="form-control"
              required minlength="2" maxlength="50"
              pattern="[a-z][a-z0-9_]{1,49}"
              <?php if ($isEdit) { echo 'readonly'; } ?>
              value="<?= esc($roleKey); ?>"
          ...
          <div class="col-12">
            <div class="form-check form-switch">
              <input class="form-check-input" type="checkbox" role="switch"
                id="role_is_admin_scope" name="is_admin_scope" value="1"
                <?php if ($isAdminScope) { echo 'checked'; } ?>>
