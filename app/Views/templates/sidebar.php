	...
	<div class="brand">
		<?php if ($appLogo !== '') { ?>
			<img src="<?= base_url($appLogo); ?>?v=<?= esc(app_setting('asset_version', '1')); ?>" alt="Logo"
				class="brand-logo-img" style="max-width: 30px; max-height: 30px; object-fit: contain;">
		<?php } else { ?>
...
<div class="main">
	<header class="topbar">
		<div class="topbar-left">
			<button type="button" class="sidebar-toggle" id="sidebarToggle" aria-label="Open navigation menu"
		...
		<div class="topbar-right">
			...
			<div class="dropdown global-date-dropdown me-2">
				<button type="button" class="btn btn-topbar-filter dropdown-toggle" id="globalDateRangeToggle"
					data-bs-toggle="dropdown" aria-expanded="false" data-bs-auto-close="outside">
					<i class="bi bi-calendar3 me-2"></i>
					<span id="globalDateRangeLabel"><?= esc($globalDateLabel); ?></span>
				</button>
				<div class="dropdown-menu dropdown-menu-end p-0 global-date-menu-premium"
					aria-labelledby="globalDateRangeToggle">
					<div id="globalDateStateStore" data-preset="<?= esc($globalDatePreset); ?>"
						data-start="<?= esc($globalDateStart); ?>" data-end="<?= esc($globalDateEnd); ?>"
					...
					<div class="premium-picker-container">
						...
						<div class="picker-presets">
							...
							foreach ($presets as $pKey => $pLabel) {
								...
								?>
								<button type="button" class="preset-btn <?= $activeClass; ?>"
									data-preset="<?= esc($pKey); ?>"><?= esc($pLabel); ?></button>
							<?php } ?>
						</div>
						...
						<div class="picker-main">
							<div class="calendars-wrapper">
								...
								<div class="calendar-panel left-calendar" data-calendar-side="left">
									<div class="calendar-header">
										<button type="button" class="btn-cal-nav prev-month"><i
												class="bi bi-chevron-left"></i></button>
										<span class="month-year-label"></span>
										<button type="button" class="btn-cal-nav next-month" style="visibility: hidden;"><i
								...
								<div class="calendar-panel right-calendar" data-calendar-side="right">
									<div class="calendar-header">
										<button type="button" class="btn-cal-nav prev-month" style="visibility: hidden;"><i
												class="bi bi-chevron-left"></i></button>
										<span class="month-year-label"></span>
										<button type="button" class="btn-cal-nav next-month"><i
												class="bi bi-chevron-right"></i></button>
									</div>
									<div class="calendar-weekdays">
										<span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span>
							...
							<div class="picker-footer">
								<div class="footer-reset">
									<button type="button" class="btn btn-sm btn-link text-decoration-none text-danger ps-0"
										id="globalDateResetBtn">Reset</button>
								</div>
								<div class="footer-buttons">
									<button type="button" class="btn btn-sm btn-outline-cancel"
										id="globalDateCancelBtn">Cancel</button>
									<button type="button" class="btn btn-sm btn-apply-premium"
										id="globalDateApplyBtn">Apply</button>
								</div>
							</div>
						</div>
					</div>
			...
			<button type="button" id="themeToggle" class="theme-toggle"
				data-update-url="<?= site_url('users/update_theme'); ?>" title="Toggle light / dark theme"
			...
			<div class="dropdown topbar-bell-dropdown">
				<button type="button" class="topbar-bell" id="topbarBell" title="<?= esc($actionableTip); ?>"
					aria-label="<?= esc($actionableTip); ?>" data-bs-toggle="dropdown" data-bs-auto-close="outside"
					aria-expanded="false" data-actionable-url="<?= esc(site_url('notifications/actionable_count')); ?>"
					data-recent-url="<?= esc(site_url('notifications/recent')); ?>" data-bell-target="<?= esc($bellUrl); ?>">
	...
	<main class="content" id="mainContent">
		...
		<?php if ($okMsg) { ?>
			<div class="alert alert-success alert-dismissible fade show"><?= esc($okMsg); ?><button type="button"
					class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
		<?php } ?>
		<?php if ($errMsg) { ?>
			<div class="alert alert-danger alert-dismissible fade show"><?= esc($errMsg); ?><button type="button"
					class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
		<?php } ?>