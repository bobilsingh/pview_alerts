...
<?php if ($view === 'list') { ?>
    ...
    <div class="card">
      <div class="card-body p-0">
        <table id="flowsTable" class="table align-middle mb-0" data-table-url="<?= site_url('flows/data_table'); ?>">
          <thead>
            <tr>
              <th>Name</th>
              <th>Project</th>
          ...
          <div class="card-body">
            <form method="post" action="<?= esc($action); ?>" data-loading-form="1" data-dirty-guard="1">
              ...
              <?php foreach ($projects as $p) { ?>
                      <option value="<?= (int) $p['id']; ?>" <?php if ((int) $p['id'] === $currentProjectId) {
                           echo 'selected';
                         } ?>>
                    <?= esc($p['name']); ?>
                      </option>
              ...
              <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" required maxlength="200" data-char-counter="1"
                  value="<?= esc($flowName); ?>">
              </div>

              <div class="mb-3">
                <label class="form-label">TAT Level Count
                  <small class="text-muted fw-normal ms-1">— how many escalation levels before a ticket is flagged as escalated
                    (1–4)</small>
                </label>
              <?php
              $currentTatLevel = 4;
              ...
              <?php foreach ([1, 2, 3, 4] as $lvl) { ?>
                      <option value="<?= $lvl; ?>" <?php if ($currentTatLevel === $lvl) {
                          echo 'selected';
                        } ?>>
                        L<?= $lvl; ?>         <?php if ($lvl === 4) {
                                     echo '(default)';
    ...
    <?php } ?>
            ...
            <div class="row g-3">
                ...
                <div class="card h-100">
                  ...
                  <div class="card-body p-2">
                    <ul id="stateList" class="state-list list-unstyled mb-0"
                      data-reorder-url="<?= site_url('flows/reorder_states'); ?>" data-flow-id="<?= (int) $flow['id']; ?>"
              ...
              <?php foreach ($states as $i => $s) { ?>
                      ...
                      ?>
                          <li class="state-item<?= $editingCls; ?>" data-id="<?= $sid; ?>" data-parent-id="<?= $parentStateIdVal; ?>">
                          ...
                          <?php if (empty($s['is_final']) && $l1c === 0) { ?>
                                      <span class="badge bg-warning text-dark"
                                        title="No L1 operators assigned — tickets in this state cannot be auto-assigned"><i
                                          class="bi bi-exclamation-triangle-fill"></i> No L1 pool</span>
                          <?php } ?>
                                </div>
                                <div class="d-flex align-items-center gap-1 flex-wrap mt-1 small">
                                  <span class="badge bg-light text-dark border">L1:
                            <?= (int) $s['l1_tat_minutes']; ?>m·<?= $l1c; ?>u</span>
                                  <span class="badge bg-light text-dark border">L2:
                            <?= (int) $s['l2_tat_minutes']; ?>m·<?= $l2c; ?>u</span>
                                  <span class="badge bg-light text-dark border">L3:
                            <?= (int) $s['l3_tat_minutes']; ?>m·<?= $l3c; ?>u</span>
                                  <span class="badge bg-light text-dark border">L4:
                            <?= (int) $s['l4_tat_minutes']; ?>m·<?= $l4c; ?>u</span>
                                </div>
                        <?php if (!empty($sbwd)) { ?>
                        ...
                        <?php } ?>
                              ...
                              <div class="d-flex flex-column gap-1 text-end">
                                ...
                                <a href="<?= site_url('flows/delete_state/' . $sid); ?>" class="btn btn-sm btn-outline-danger"
                                  data-method="post" data-confirm-message="Delete this state?" aria-label="Delete this state">
              ...
              <?php } ?>
                      ...
                      <div class="d-flex gap-3 mb-3">
                        <div class="form-check form-switch">
                          <input class="form-check-input" type="checkbox" name="is_initial" id="isInitial" <?= $initialChecked; ?>>
                          <label class="form-check-label" for="isInitial">Initial state</label>
                        </div>
                        <div class="form-check form-switch">
                          <input class="form-check-input" type="checkbox" name="is_final" id="isFinal" <?= $finalChecked; ?>>
                  ...
                  $parentCandidates = array_filter($states, function ($sp) use ($isEditState, $editSid) {
                    if ($isEditState && (int) $sp['id'] === $editSid) {
                      return false;
                    }
                    if (!empty($sp['is_final'])) {
                      return false;
                    }
                    return true;
                  });
                  if (!empty($parentCandidates)) {
                    ...
                    ?>
                          <div class="mb-3" id="parentStateWrap">
                            <label class="form-label">Parent State <small class="text-muted">(defines branching
                                structure)</small></label>
                      ...
                      <?php } ?>
                            ...
                            <div class="form-text">Attach as a child of another state to create a branch. Leave blank for root-level
                              states (initial, closing).</div>
                          </div>
              <?php } ?>
                  ...
                  if (!empty($bwdCandidates)) {
                    ?>
                          <div class="mb-3">
                            <label class="form-label">Allowed Backward States <small class="text-muted">(send-back
                                targets)</small></label>
                      ...
                      <?php } ?>
                            ...
                            <div class="form-text">Operators can send tickets back to these states for rework. Leave empty to disable
                              backward movement from this state.</div>
                          </div>
              <?php } ?>
                  ...
                  for ($lvl = 1; $lvl <= 4; $lvl++) {
                    ...
                    ?>
                        ...
                        <?php foreach ($users as $u) {
                          ...
                          ?>
                                    <option value="<?= esc((string) $u['user_id']); ?>" <?= $sel; ?>><?= esc($u['name']); ?> -
                              <?= esc($u['email']); ?></option>
                        <?php } ?>
                              </select>
                            </div>
                            <div class="col-5">
                              ...
                              <input type="number" min="1" name="l<?= $lvl; ?>_tat_minutes" class="form-control"
                                value="<?= $tatVal; ?>">
                            </div>
                          </div>
              <?php } ?>
