...
<?php if ($view === 'list') { ?>
  ...
  <div class="card">
    <div class="card-body p-0">
      <table id="alertsTable" class="table align-middle mb-0" data-table-url="<?= site_url('alerts/data_table'); ?>">
        <thead>
          <tr>
            <th>Name</th>
            <th>Project</th>
        ...
        <form method="post" action="<?= esc($action); ?>" data-loading-form="1" data-dirty-guard="1">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <input type="text" name="name" class="form-control" required maxlength="200" autofocus data-char-counter="1"
                value="<?= esc($alertName); ?>">
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="threshold_value" class="form-control" value="<?= esc($thresholdValue); ?>">
            </div>
            <div class="col-md-6">
              <label class="form-label">Threshold Unit</label>
              <input type="text" name="threshold_unit" class="form-control" placeholder="%, ms, errors/min"
                value="<?= esc($thresholdUnit); ?>">
            </div>
            ...
            <div class="col-12">
              ...
              <?php foreach ($users as $u) { ?>
                <?php $uVal = (string) $u['user_id']; ?>
                  <option value="<?= esc($uVal); ?>" <?php if (in_array($uVal, $existingNotify, true)) {
                      echo 'selected';
                    } ?>>
          ...
          <?php if ($isEdit) { ?>
              <div class="col-12">
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" name="is_active" id="isActive" <?php if ($isActive) {
                    echo 'checked';
                  } ?>>
              ...
              <form method="post" action="<?= site_url('escalation/save'); ?>">
                <div class="mb-3">
                  ...
                  <select name="flow_id" id="escalationFlowSelect" class="form-select"
                    data-load-target="#escalationStateSelect" data-load-url="<?= site_url('escalation/states_by_flow'); ?>"
                    data-item-type="state" data-empty-text="Select flow first" data-default-text="Select state"
                    data-loading-text="Loading states..." data-no-data-text="No states in this flow"
                    data-error-text="Failed to load states" required>
                    <option value="">Select flow</option>
                <?php foreach ($flows as $f) { ?>
            ...
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header border-bottom-0 pb-0">
                  <h5 class="modal-title" id="newApiKeyModalLabel"><i class="bi bi-key-fill text-info"></i> New API Key Generated
                  </h5>
                </div>
                <div class="modal-body">
                  <p class="text-warning mb-3"><i class="bi bi-exclamation-triangle"></i> Please copy this key immediately. You
                    will not be able to see it again!</p>
                  <div class="input-group mb-2">
                    <input type="text" id="newApiKeyValue" class="form-control font-monospace" value="<?= esc($newKeyValue); ?>"
                      readonly>
                ...
                <?php foreach ($keys as $k) { ?>
                    ...
                    ?>
                      ...
                      <?php } ?>
                          ...
                          <td class="text-end">
                            <a class="btn btn-sm btn-light" href="<?= site_url('api_keys/toggle/' . $k['id']); ?>"
                              data-method="post" data-confirm-message="Toggle this API key's active state?">
