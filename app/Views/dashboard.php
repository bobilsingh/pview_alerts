...
<div class="page-head">
  ...
  <div class="d-flex align-items-center gap-2">
    ...
    <button type="button" class="btn btn-light" id="dashCustomizeToggle" aria-expanded="false"
      aria-controls="dashCustomizePanel" title="Customize dashboard">
...
<div class="kpi-grid">
  <?php if ($show['open'] === 1) { ?>
      <a class="kpi-link" href="<?= site_url('tickets/all?status=active'); ?>"
  ...
  <?php if ($show['critical'] === 1) { ?>
      <a class="kpi-link" href="<?= site_url('tickets/all?alert_type=critical&status=active'); ?>"
  ...
  <?php } ?>
  <?php if ($show['major'] === 1) { ?>
      <a class="kpi-link" href="<?= site_url('tickets/all?alert_type=major&status=active'); ?>"
  ...
  <?php if ($show['resolved'] === 1) { ?>
      <a class="kpi-link" href="<?= site_url('tickets/all?status=resolved'); ?>"
  ...
  <div class="col-xl-8">
    <div class="card chart-card">
      ...
      <div class="chart-wrap trend-chart-wrap">
        <canvas id="trendChart" data-chart="trend" data-labels="<?= esc(implode('||', $trendLabels), 'attr'); ?>"
          data-values="<?= esc(implode(',', array_map('strval', $trendValues)), 'attr'); ?>"
          data-dates="<?= esc(implode('||', $trendDates), 'attr'); ?>"></canvas>
      </div>
    </div>
  </div>
  <div class="col-xl-4">
    <div class="card chart-card severity-card">
      ...
      <div class="chart-wrap severity-chart-wrap">
        <canvas id="severityChart" data-chart="severity" data-info="<?= $infoCount; ?>"
          data-major="<?= $majorMixCount; ?>" data-critical="<?= $criticalMixCount; ?>"></canvas>
      </div>
    </div>
  </div>
</div>
...
<div class="card recent-card">
  <div class="card-header">
    ...
    <?php if ((int) $tatBreached > 0) { ?>
        <span class="badge bg-danger ms-1"><i class="bi bi-exclamation-triangle-fill"></i> <?= (int) $tatBreached; ?>
          Escalated</span>
    <?php } ?>
    ...
    <table class="table align-middle mb-0">
      ...
      <tbody>
        ...
        <?php foreach ($recentTickets as $t) { ?>
            ...
            ?>
            <tr class="<?= $rowClass; ?>">
              <td><a href="<?= site_url('tickets/detail/' . $t['alarm_id']); ?>"
                  class="alarm-id"><?= esc($t['alarm_id']); ?></a></td>
              <td>
                <div class="fw-semibold text-truncate" style="max-width:260px;" title="<?= esc($t['title']); ?>">
                  <?= esc($t['title']); ?></div>
              ...
              <td><span class="tat-countdown" data-tat-expires="<?= esc($expires); ?>"
                  data-tat-total-ms="<?= (int) (tat_total_minutes($t) * 60000); ?>"></span></td>
            </tr>
        <?php } ?>
      </tbody>
    </table>
