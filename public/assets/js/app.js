...
$appDocument.ready(function () {
	...
	if (typeof setupDataTablesDefaults === "function") {
...
function autoDismissFlashAlerts($scope) {
	var $root = $scope && $scope.length ? $scope : $appDocument;
	var $flashAlerts = $root.find(".alert-success, .alert-danger, .alert-warning").filter(".alert-dismissible");
...
function initDynamicContent() {
	autoDismissFlashAlerts($("#mainContent"));

	// --- Date range widget ---
	if (typeof initGlobalDateFilter === "function") {
...
function initCustomTooltips() {
	...
	$appDocument.off("mouseenter.appTooltip", "[title]").on("mouseenter.appTooltip", "[title]", function () {
		var $target = $(this);
	...
	$appDocument.off("mouseleave.appTooltip", "[data-tooltip]").on("mouseleave.appTooltip", "[data-tooltip]", function () {
		var $target = $(this);
	...
	$appWindow.off("scroll.appTooltip").on("scroll.appTooltip", function () {
		if ($tooltipEl) {
			$tooltipEl.remove();
			$tooltipEl = null;
			if ($activeTarget) {
...
function initAutoLogout() {
	...
	function doLogout() {
		// POST to /logout without requiring a token; CSRF is not enabled for this app.
...
function initFormValidation() {
	...
	$appDocument.off("blur.validation", "form [required]").on("blur.validation", "form [required]", function () {
		var $field = $(this);
	...
	$appDocument.off("input.validation change.validation", "form [required].is-invalid").on("input.validation change.validation", "form [required].is-invalid", function () {
		validateField($(this));
	});
}
